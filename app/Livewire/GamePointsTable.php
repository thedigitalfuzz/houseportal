...
// Delete recharge
    ...
    public function render()
    {
        ...
        // Get bonus_added grouped by game_id + date
        $bonusData = \App\Models\Transaction::selectRaw('game_id, DATE(transaction_date) as date, SUM(bonus_added) as total_bonus')
            ->groupBy('game_id', 'date')
            ->get()
            ->keyBy(fn($item) => $item->game_id . '_' . $item->date);

        // --- FILTER: show only rows where main points are entered ---
        $recordsWithPoints = $records->filter(fn($r) => $r->points > 0);

        $records->transform(function ($r) use ($bonusData) {
            $key = $r->game_id . '_' . \Carbon\Carbon::parse($r->date)->format('Y-m-d');

            $r->bonus_added = $bonusData[$key]->total_bonus ?? 0;
