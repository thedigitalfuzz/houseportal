<div>
    ...
    <div class="flex flex-col md:flex-row justify-between mb-4 gap-2">
        ...
        <div class="flex gap-2">
            <a href="{{route('monthly-wallet-performance')}}"
               class="px-4 py-2 bg-blue-600 text-white rounded">
                Monthly Performance
            </a>

            <a href="{{route('overall-wallet-performance')}}"
...
<div class="grid grid-cols-1">
    <div class="mb-6 border p-4 bg-white">
        ...
        <h3 class="font-bold mb-3">Date: {{ $date }}</h3>
        ...
        <div class="bg-white rounded shadow overflow-x-auto">
            <table class="min-w-full table-auto">
                ...
                @foreach($data[$date] as $row)
                    <tr class="border-t">
                        <td class="p-3 text-left">{{ $row['rank'] }}</td>
                        <td class="p-3 text-left">{{ $row['agent'] }}</td>
                        <td class="p-3 text-left">{{ $row['wallet_name'] }}</td>
                        <td class="p-3 text-left">{{ $row['wallet_remarks'] }}</td>
                ...
                @endforeach
                @php
                    $totalTxn = collect($data[$date])->sum('count');
                        $totalCashin = collect($data[$date])->sum('cashin');
                        $totalCashout = collect($data[$date])->sum('cashout');
                        $totalNet = $totalCashin - $totalCashout;
                        $topRow = $data[$date][0] ?? null;
                @endphp
